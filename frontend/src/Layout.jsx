...
export default function Layout({ myBlockId, account, connected, onConnect }) {
  ...
  return (
    <div>
      {/* Navbar */}
      <nav className="bg-slate-800/80 backdrop-blur-md shadow-lg">
        ...
        <div className="max-w-7xl mx-auto flex justify-between px-4 py-3">
          ...
          <div className="hidden md:flex items-center gap-4 w-full justify-end">
            <Link
              to="/"
              className="px-4 py-2 rounded-full bg-gradient-to-r text-white font-semibold"
            >
              Home
            </Link>
            ...
            <Link
              to="/dashboard"
              className="px-4 py-2 rounded-full hover:opacity-90 mr-8"
            >
              Dashboard
            </Link>
            ...
            <button
              className="px-3 py-1 rounded-full bg-slate-800/50 mr-8"
              onClick={onConnect}
            >
              Connect
            </button>
        ...
        {menuOpen && (
          <div className="md:hidden bg-slate-900/95 px-4 pb-4 pt-2 rounded-b-2xl shadow-lg">
            <Link
              to="/"
              onClick={() => handleNav("/")}
              className="block px-4 py-2 rounded hover:bg-slate-700 text-white font-semibold"
            >
              Home
            </Link>
            ...
            <Link
              ...
              className="block px-4 py-2 rounded hover:bg-slate-700 text-white"
            >
              Dashboard
            </Link>
            {isLoggedIn ? (
            ...
            <button
              className="block w-full text-left px-4 py-2 rounded bg-slate-800/50 mt-2"
              onClick={() => {
                onConnect && onConnect();
                setMenuOpen(false);
              }}
